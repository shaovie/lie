...
/* **********************
   ...
   ********************** */
var _swfUploadQueueArray = new Array();
...
function fileQueued(file) {
	//记录文件队列ID
	_swfUploadQueueArray.push(file.id);
	try {
...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	...
	if(current_url.indexOf('\/index.php\/ShareAction\/') > 0) {
		var imgs = numFilesSelected + $('#imgs input').length;
		if( imgs > 9) {
		   alert('只能上传9张图片');
		   //取消上传队列
		   for(var i=0;i<_swfUploadQueueArray.length;i++) {
		   	  this.cancelUpload(_swfUploadQueueArray[i]);
		   }
		   return false;
		}
	}
...
function uploadError(file, errorCode, message) {
	try {
		...
		case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
			...
			progress.setStatus("全部取消");
			progress.setCancelled();
			break;
		case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:
			progress.setStatus("全部停止");
			break;
		default:
